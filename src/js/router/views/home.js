...
if (allActivePosts.length > 0) {
  sortedPosts = allActivePosts.sort((a, b) => new Date(a.endsAt) - new Date(b.endsAt));
...
while (hasMore) {
  ...
  if (postList.length === 0) {
    ...
    allActivePosts = [...allActivePosts, ...postList.filter(post => new Date(post.endsAt) > new Date())];
    page++;
  }
}
}
...
auctionEnds.textContent = `Auction ends: ${new Date(postData.endsAt).toLocaleString([], {
  year: "2-digit",
  month: "2-digit",
  day: "2-digit",
  hour: "2-digit",
